...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] text-brand-text flex flex-col font-sans">
      <div className="p-6">
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={submit} data-testid="login-form" className="space-y-5 bg-[#141312] border border-brand-line p-8">
            <div>
              <label className="text-xs uppercase tracking-[0.15em] text-brand-muted">Email</label>
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                data-testid="login-email"
                className="w-full mt-2 bg-[#0A0A0B] border border-brand-line px-4 py-3 text-brand-text focus:border-brand-bronze focus:outline-none" />
            </div>
            <div>
