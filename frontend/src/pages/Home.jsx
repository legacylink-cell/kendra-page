...
const Home = () => {
  ...
  const inputCls = "w-full bg-[#ECFDFF] border border-[#D6EEF1] rounded-xl px-4 py-3.5 text-[#0B3B4A] placeholder:text-[#4A5B60]/50 focus:border-[#0FB6C4] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0FB6C4]/30 transition-colors";

  return (
    <div id="top" className="bg-[#FFFFFF] text-[#0B3B4A] font-body selection:bg-[#0FB6C4] selection:text-white">
      <MarketingNav />
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-12 gap-10 md:gap-8 items-center">
          <div className="md:col-span-7">
            <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
              className="text-xs tracking-[0.25em] uppercase font-semibold text-[#0FB6C4] mb-6">/ Strength for women · by a woman /</motion.p>
            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }}
              className="font-display text-5xl sm:text-6xl md:text-7xl leading-[0.98] tracking-tight text-[#0B3B4A]">
              Real strength.
              <br />
              <span className="italic text-[#FF6B6B]">Built to last.</span>
            </motion.h1>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }}
              ...
              <a href="#contact" data-testid="hero-cta-start" onClick={() => track("click", { label: "Hero — Start Here" })}
                className="group inline-flex items-center gap-2 bg-[#0FB6C4] text-white px-8 py-4 rounded-full text-sm uppercase tracking-[0.15em] hover:bg-[#0C97A3] hover:scale-[1.03] transition-transform">
                Start Here
                <ArrowUpRight className="h-4 w-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
              </a>
              <a href="#programs" data-testid="hero-cta-programs"
                className="inline-flex items-center gap-2 border border-[#0B3B4A]/25 text-[#0B3B4A] px-8 py-4 rounded-full text-sm uppercase tracking-[0.15em] hover:border-[#0FB6C4] hover:text-[#0FB6C4] transition-colors">
                View Programs
              </a>
            </motion.div>
          </div>
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.9, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
            ...
            <div className="absolute -bottom-5 -left-3 sm:left-6 bg-[#0B3B4A] text-[#FFFFFF] px-6 py-4 rounded-2xl shadow-[0_10px_40px_rgba(28,27,26,0.15)]">
              ...
              <p className="text-[10px] uppercase tracking-[0.2em] mt-1 text-[#FFFFFF]/70">Years coaching women</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      {/* ABOUT KENDRA */}
      <section id="about" className="scroll-mt-32 py-20 md:py-32 px-6 md:px-12 bg-[#ECFDFF]">
        <div className="max-w-7xl mx-auto grid md:grid-cols-12 gap-12 md:gap-16 items-center">
          ...
          <Reveal delay={0.15} className="md:col-span-7 order-1 md:order-2">
            ...
            <p className="text-[#4A5B60] text-lg leading-relaxed mb-5">
              I coach my clients to dig deep in discovering their <span className="text-[#0B3B4A] font-medium">'why'</span> behind their
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {["Women-only coaching", "Personalized programming", "Form-first strength", "Real accountability"].map((f) => (
                <div key={f} className="flex items-center gap-3 text-sm text-[#0B3B4A]">
                  <span className="h-6 w-6 rounded-full bg-[#0FB6C4]/15 flex items-center justify-center shrink-0">
                    <Check className="h-3.5 w-3.5 text-[#0FB6C4]" />
                  </span>
                  {f}
                </div>
              ))}
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 md:gap-20">
          <Reveal>
            ...
            <ul className="space-y-5">
              {aboutYou.map((t) => (
                <li key={t} className="flex gap-4 text-[#4A5B60] leading-relaxed">
                  <span className="mt-2.5 h-1.5 w-1.5 rounded-full bg-[#0FB6C4] shrink-0" />
                  <span>{t}</span>
                </li>
              ))}
            </ul>
            <p className="mt-10 font-medium text-[#0B3B4A] text-lg">
              You need coaching that's customized to you, intelligent, and built to last.
            </p>
          </Reveal>
          <Reveal delay={0.15}>
            <div className="bg-[#F3FBFC] rounded-[32px] p-8 md:p-12 h-full">
              ...
              <ul className="space-y-5">
                {different.map((t) => (
                  <li key={t} className="flex gap-4 text-[#4A5B60] leading-relaxed">
                    <Check className="h-5 w-5 text-[#0FB6C4] shrink-0 mt-0.5" />
                    <span>{t}</span>
                  </li>
                ))}
              </ul>
      ...
      {/* PROGRAMS / SERVICES */}
      <section id="programs" className="scroll-mt-32 py-20 md:py-32 px-6 md:px-12 bg-[#ECFDFF]">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mt-16">
            {programs.map((p, i) => (
              <Reveal key={p.title} delay={(i % 3) * 0.1}>
                <div data-testid={`program-${i}`} className="group h-full bg-[#F3FBFC] rounded-[28px] p-8 md:p-9 flex flex-col">
                  <div className="flex items-center justify-between mb-8">
                    <span className="h-12 w-12 rounded-full bg-[#FFFFFF] flex items-center justify-center">
                      <p.icon className="h-5 w-5 text-[#0FB6C4]" />
                    </span>
                    <span className="text-[10px] uppercase tracking-[0.2em] text-[#4A5B60]">{p.tag}</span>
                  </div>
                  <h3 className="font-display text-2xl md:text-3xl mb-4 text-[#0B3B4A]">{p.title}</h3>
                  <p className="text-[#4A5B60] text-sm leading-relaxed flex-1">{p.desc}</p>
                  <button type="button" onClick={() => enquire(p.title)} data-testid={`program-enquire-${i}`}
                    className="mt-8 inline-flex items-center justify-center gap-2 bg-[#0FB6C4] text-white px-6 py-3 rounded-full text-xs uppercase tracking-[0.15em] hover:bg-[#0C97A3] hover:scale-[1.03] transition-transform self-start">
      ...
      <section id="results" className="scroll-mt-32 py-20 md:py-32 px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          <Reveal>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-14">
              <div>
                ...
                <h2 className="font-display text-4xl sm:text-5xl md:text-6xl tracking-tight leading-[1.02]">Real women. <span className="text-[#FF6B6B]">Real strength.</span></h2>
              </div>
              <a href="#contact" className="text-[#0FB6C4] hover:text-[#0C97A3] text-sm inline-flex items-center gap-2 uppercase tracking-[0.12em]">
          ...
          {/* Beyond the studio */}
          <Reveal>
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-14 items-center mt-16 bg-[#F3FBFC] rounded-[32px] p-6 lg:p-12">
              <div className="relative overflow-hidden rounded-[24px]">
                <img src={IMG.marathon} alt="Kendra at the 2024 half marathon finish line" className="w-full aspect-[4/3] object-cover" />
                <div className="absolute bottom-4 left-4 bg-[#0FB6C4] text-white px-4 py-2 rounded-full text-[11px] uppercase tracking-[0.2em]">2024 · 13.1 finishers</div>
              </div>
              <div>
                ...
                <p className="text-[#4A5B60] leading-relaxed mb-4">
                ...
                <p className="text-[#4A5B60] leading-relaxed">
      ...
      {/* TESTIMONIALS */}
      <section className="py-20 md:py-32 px-6 md:px-12 bg-[#0B3B4A] text-[#FFFFFF]">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs tracking-[0.25em] uppercase font-semibold text-[#0FB6C4] mb-8">/ Client love /</p>
            ...
            <AnimatePresence mode="wait">
              <motion.blockquote key={ti}
                ...
                <footer className="mt-8 not-italic">
                  ...
                  <span className="block font-body text-xs text-[#FFFFFF]/50 mt-1 tracking-[0.12em] uppercase">{testimonials[ti].r}</span>
                </footer>
              </motion.blockquote>
            </AnimatePresence>
          </div>
          <div className="flex items-center justify-center gap-4 mt-10">
            <button data-testid="testimonial-prev-button" aria-label="Previous testimonial"
              ...
              className="h-12 w-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-[#0FB6C4] hover:border-[#0FB6C4] transition-colors">
      ...
      {/* CTA BANNER */}
      <section className="py-20 md:py-28 px-6 md:px-12 bg-[#0FB6C4] text-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <a href="#contact" data-testid="cta-banner-button"
            className="mt-10 inline-flex items-center gap-2 bg-[#0B3B4A] text-white px-8 py-4 rounded-full text-sm uppercase tracking-[0.15em] hover:scale-[1.03] transition-transform">
      ...
      {/* CONTACT */}
      <section id="contact" className="scroll-mt-32 py-20 md:py-32 px-6 md:px-12 bg-[#ECFDFF]">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-14 lg:gap-20">
          <Reveal>
            ...
            <p className="text-[#4A5B60] text-lg leading-relaxed max-w-md">
          ...
          <Reveal delay={0.15}>
            <form onSubmit={submit} data-testid="lead-form" className="space-y-4">
              ...
              <button type="submit" disabled={sending} data-testid="lead-submit"
                className="w-full bg-[#0FB6C4] text-white py-4 rounded-full text-sm uppercase tracking-[0.15em] hover:bg-[#0C97A3] transition-colors disabled:opacity-60">
